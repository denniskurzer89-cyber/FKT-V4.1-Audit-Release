...
\addbibresource{references.bib} % ODER wie auch immer Ihre .bib-Datei heißt
\begin{document}
Hallo Welt!
\end{document}
...
\subsection{Digitale Nachlass- und Kontinuitätsklausel (Finale Autorität und Axiomatik)}
[...]
\textbf{Zwingende Axiomatische Erweiterungsbedingung:} Eine Erweiterung oder Modifikation der FKT-Theorie nach dem Ableben des Erstautors ist \textbf{ausschließlich} dann zulässig, wenn die vorgeschlagene Erweiterung auf \textbf{fungierten, konsistenten mathematischen Berechnungen} beruht, die mindestens folgende Eigenschaft(en) aufweisen:
\begin{enumerate}
    \item Die Erweiterung muss die \textbf{dynamische Reproduktion} und die Stabilität des empirisch abgeleiteten Best-Fit-Wertes $w_X = -0.90 \pm 0.05$ (68\% CI) über alle relevanten Epochen der Kosmologie gewährleisten.
    \item Die zugrunde liegenden mathematischen Berechnungen müssen die \textbf{fraktale Kausalität} und die Prinzipien der \textbf{Skaleninvarianz} des FKT-Frameworks strikt bewahren.
    \item Die Erweiterung muss vollständig mit dem \textbf{EYRQ-Modell} und dem \textbf{Fütterer-Bulk-Effekt} kompatibel sein und darf \textbf{keine neuen, ungebundenen (arbiträren) freien Parameter} einführen, die die Prädiktionskraft der Theorie schwächen.
\end{enumerate}
Diese Klausel, als unumstößliche \textbf{Klaus-Einschließung}, sichert die wissenschaftliche Kohärenz der Theorie für die Zukunft.
